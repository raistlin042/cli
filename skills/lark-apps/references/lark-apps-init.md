...
**端到端/已授权直接派生默认目录**：当请求是端到端一气呵成（如"做完部署上线"/"本地跑起来"）或已预授权时，按应用名派生语义化目录（如 `./crm-app`）直接 `+init --dir`，不停下来问；仅当目标目录已存在/非空/冲突时，才回退下面的目录选择话术。
...
- 端到端/已授权场景例外：可用应用名派生目录（`./<app-name>`）直接继续；目录冲突/非空时才回交互。
