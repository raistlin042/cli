...
lark-cli apps +create --name "团队任务看板" --app-type full_stack \
...
|---|---|---|
| `--name <str>` | ✅ | 应用显示名 |
| `--app-type <enum>` | ✅ | 应用类型，可选值：`html` 或 `full_stack`（均区分大小写） |
...
| 需要后端能力：数据库、登录鉴权、API、表单提交存储、用户系统、增删改查、持久化、服务端逻辑、"全栈"、"带后台 / 后台管理" | 全栈应用 | `full_stack` |
...
识别到用户需要全栈/带后端能力的应用后（如需要登录、数据库、增删改查、用户系统等），使用 `--app-type full_stack`，并从用户描述生成 `name` 和 `description`：
