...
8. **一键初始化本地开发仓库（`apps +init`）** → 必读 [`lark-apps-init.md`](references/lark-apps-init.md)（把 `+git-credential-init` → `git clone` → 切 `sprint/default` 串成一步的便捷封装；想手动控制每步时改用第 2 / 7 条的原生 git 流程）
## 身份与一次性授权
...
| [`+init`](references/lark-apps-init.md) | 一键初始化本地开发仓库（编排 `+git-credential-init` → clone → 切 `sprint/default`；`--dir` 须为 cwd 内相对路径） | 本期新增 |
