...
// 6 基础 + 4 publish + 1 env-pull + 4 db（table-list/table-schema/sql/dev-init）+ 3 git-credential = 18。
func TestAppsShortcuts_Returns18(t *testing.T) {
	got := Shortcuts()
	if len(got) != 18 {
		t.Fatalf("Shortcuts() returned %d entries, want 18", len(got))
	}
}
