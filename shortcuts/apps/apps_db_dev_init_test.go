...
func TestAppsDBDevInit_WithYesPostsSyncData(t *testing.T) {
	factory, stdout, reg := newAppsExecuteFactory(t)
	stub := &httpmock.Stub{
		Method: "POST",
		URL:    "/open-apis/spark/v1/apps/app_x/db_dev_init", // URL 仍走 multi_env，CLI 命令名 +db-dev-init
	...
	reg.Register(stub)
	if err := runAppsShortcut(t, AppsDBDevInit,
		[]string{"+db-dev-init", "--app-id", "app_x", "--sync-data", "--yes", "--as", "user"},
...
func TestAppsDBDevInit_PrettyEmitsAllFourLines(t *testing.T) {
	factory, stdout, reg := newAppsExecuteFactory(t)
	...
	if err := runAppsShortcut(t, AppsDBDevInit,
		[]string{"+db-dev-init", "--app-id", "app_x", "--sync-data", "--yes", "--format", "pretty", "--as", "user"},
		factory, stdout); err != nil {
		t.Fatalf("execute err=%v", err)
	}
	got := stdout.String()
...
func TestAppsDBDevInit_DryRunNoConfirm(t *testing.T) {
	factory, stdout, _ := newAppsExecuteFactory(t)
	if err := runAppsShortcut(t, AppsDBDevInit,
		[]string{"+db-dev-init", "--app-id", "app_x", "--dry-run", "--as", "user"},
		factory, stdout); err != nil {
